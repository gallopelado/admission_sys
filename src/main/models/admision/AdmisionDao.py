...
class AdmisionDao:
    ...
    def getProfesionales(self):
        ...
        try:
            ...
            cursor.execute("SELECT p.prof_codigo_medico, e.espec_id, e.espec_descripcion, u.usu_nombres, u.usu_apellidos FROM profesional p LEFT JOIN usuarios u ON u.usu_codigo_usuario = p.prof_codigo_medico LEFT JOIN especialidad e ON e.espec_id = p.espec_id WHERE p.prof_activo = 't';")
    ...
    def guardarPaciente(self, pacienteDto):
        try:
            ...
            pacienteDto.sitlabId, pacienteDto.pacLatitud, pacienteDto.pacLongitud, pacienteDto.pacCreacionUsuario, pacienteDto.pacCreacionFecha, pacienteDto.pacCreacionHora,))
            conexion.con.commit()
            cursor.close()
            conexion.con.close()
    ...
    def actualizarPacienteFormularioPrincipal(self, pacienteDto):
        try:
            ...
            , pac_telefono=%s, pac_direccion=%s, pac_modificacion_usuario=%s, pac_modificacion_fecha=%s, pac_modificacion_hora=%s)
            WHERE pac_codigo_paciente=%s""", 
            (pacienteDto.pacNombres, pacienteDto.pacApellidos, pacienteDto.pacSexo,
            pacienteDto.pacFechaNac, pacienteDto.pacLugarNacimiento, pacienteDto.ciuId, pacienteDto.pacCorreoElectronico, pacienteDto.nacId,
            pacienteDto.pacTelefono, pacienteDto.pacDireccion, pacienteDto.pacModificacionUsuario, pacienteDto.pacModificacionFecha, pacienteDto.pacModificacionHora,))
            conexion.con.commit()
            cursor.close()
            conexion.con.close()
    ...
    def actualizarPacienteOtrosDatos(self, pacienteDto):
        try:
            ...
            , edu_id=%s, sitlab_id=%s, pac_latitud=%s, pac_longitud=%s, pac_modificacion_usuario=%s, pac_modificacion_fecha=%s, pac_modificacion_hora=%s)
            WHERE pac_codigo_paciente=%s""", 
            (pacienteDto.segId, pacienteDto.pacHijos, pacienteDto.pacEstadoCivil, pacienteDto.eduId, pacienteDto.sitlabId,
            pacienteDto.pacLatitud, pacienteDto.pacLongitud, pacienteDto.pacModificacionUsuario, pacienteDto.pacModificacionFecha, pacienteDto.pacModificacionHora,))
            conexion.con.commit()
            cursor.close()
            conexion.con.close()
    ...
    def getDatosPacienteByCodigoPaciente(self, codigo_paciente):
        ...
        try:
            ...
            lista = []
            for item in items:
                lista.append(Pacientedto(item[0], item[1], item[2], item[3], item[4], item[5], item[6], item[7], item[8], item[9], item[10], item[11], item[12], item[13], item[14], item[15], item[16], item[17], item[18]))
            cursor.close()
            conexion.con.close()
            return lista
        except conexion.con.Error as e:
    ...
    def insertarPacienteAsignacion(self, pacienteAsignacionDto):
        ...
        try:
            ...
            insertPacienteAsignacionSQL = "INSERT INTO public.paciente_asignacion(pacasi_codigo_establecimiento, pacasi_codigo_asignacion, pac_codigo_paciente, med_id, supl_med_id, pacasi_estado, seg_id, pacasi_creacion_usuario, pacasi_creacion_fecha, pacasi_creacion_hora) VALUES(%s, %s, %s, %s, %s, 'A', %s, CURRENT_DATE, CURRENT_TIME)"
            ...
            cursor.execute(insertPacienteAsignacionSQL, (pacienteAsignacionDto.pacasiCodigoEstablecimiento, pacienteAsignacionDto.pacasiCodigoAsignacion, pacienteAsignacionDto.pacCodigoPaciente, pacienteAsignacionDto.medId, pacienteAsignacionDto.suplMedId, pacienteAsignacionDto.segId, pacienteAsignacionDto.pacasiCreacion_usuario,))
            cursor.execute(insertPreconsultaSQL, (pacienteAsignacionDto.pacasiCodigoEstablecimiento, pacienteAsignacionDto.pacasiCodigoAsignacion, pacienteAsignacionDto.pacasiCreacion_usuario,))
            cursor.execute(insertConsultaSQL, (pacienteAsignacionDto.pacasiCodigoEstablecimiento, pacienteAsignacionDto.pacasiCodigoAsignacion, pacienteAsignacionDto.pacasiCreacion_usuario,))
